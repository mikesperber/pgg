...
0.50; PGG has been known to work with versions $\ge$ 0.42.

Once you have installed Scheme48, unpack the distribution file by 
\begin{verbatim}
polihale> mkdir pgg-1.0
polihale> cd pgg-1.0 
polihale> zcat <PATH_TO>pgg-1.0.tar.gz | tar xvf -
\end{verbatim}
(with \verb+polihale>+ being the shell's prompt)
This creates the directory \texttt{pgg-1.0} in the current directory.
...
polihale> make
(echo ",bench on"; \
 echo ",config,load cogen-interfaces.scm"; \
 echo ",load-package pgg"; \
 echo ",open pgg signals pretty-print"; \
 echo ",open auxiliary pgg-library"; \
 echo ",collect"; \
 echo ",dump pgg.image \"(PGG-1.0)\""; \
 echo ",exit" ) \
| scheme48 -h 4000000
Welcome to Scheme 48 0.46 (made by peter on ...).
...
Undefined: (message-low aspaces proxies anf-specializer ...)
> [pgg
[auxiliary
[pretty-print
...
Undefined: (parse-type desc-np desc-nc desc-nt desc-type)
...
cogen-driver.scm ]
> > > Before: 202193 words free in semispace
After:  1311637 words free in semispace
> Writing pgg.image
> polihale>
\end{verbatim}
...
\begin{verbatim}
polihale> scheme48 -h 4000000 -i pgg.image
\end{verbatim}
...
be increased 
when dealing with larger programs. The
...
the system has been started in the \texttt{pgg-1.0} directory. The
...
bta-solve
bta-solve done
> (eval `(begin ,@genext) (interaction-environment))
>
\end{verbatim}
\end{small}
...
(eval `(begin ,@genext) (interaction-environment))
> (specialize '($goal 0 0 1) '(5 () values))
'($goal-1 values)
> (p *residual-program*)
((define ($goal-1 values-1) 5))
32
> (specialize '($goal 0 0 1) '((+ x y) (x y) values))
'($goal-1 values)
> (p *residual-program*)
((define ($goal-1 values-1)
   (let* ((mlet-2 (cdr values-1))
     ...
     (+ mlet-3 mlet-5))))
26
> (specialize '($goal 0 0 1) '((lambda (x) (+ x y)) (y) values))
'($goal-1 values)
> (p *residual-program*)
((define ($goal-1 values-1)
...
>                              '(dotprod 0 1 2 3)))
> (eval `(begin ,@genext) (interaction-environment))
> (specialize '($goal 0 1 2 3) '(2 v1 v2 v3))
...
> (specialize '($goal-1 0 1 2) '((111 222) v2 v3))
'(multi-memo 1 '$goal-1-1 $goal-1-1 '(0 1) (list v2 v3))
> (eval `(begin ,@*residual-program*) (interaction-environment))
> (specialize '($goal-1-1 0 1) '((333 444) v3))
'($goal-1-1-1 v3)
> (p *residual-program*)
...
\end{verbatim}
\end{small}
\item \texttt{pm} Olivier Danvy's pattern matcher
  \begin{small}
\begin{verbatim}
...
memoization functions can be defined and inserted manually
...
\begin{verbatim}
D ::= (define-memo M Number)
\end{verbatim}
Defines \texttt{M} as a unary operator indicating a memoization point at
level \texttt{Number}. For two-level specialization this number is
...
\begin{verbatim}
(specialize BindingTimeSkeleton ARGS)
\end{verbatim}
and
\begin{verbatim}
...
\begin{itemize}
\item \texttt{BindingTimeSkeleton}  is a list where the first element is a
  symbol denoting the main function of the generating extension and the remaining 
  elements are binding times its parameters. The list of binding times must be
  identical to the one given to \texttt{cogen-driver} when creating this
...
These options are accessible in module \texttt{cogen-globals}. Some of
