...
\end{itemize}
As a non-feature, the system does not have a post-processor.
...
\url{http://www.neci.nj.nec.com/homepages/kelsey/}. The current version is
0.53; PGG has been known to work with versions $\ge$ 0.42.

Once you have installed Scheme48, unpack the distribution file by 
\begin{verbatim}
hanauma> mkdir pgg-1.1
hanauma> cd pgg-1.1 
hanauma> zcat <PATH_TO>pgg-1.1.tar.gz | tar xvf -
\end{verbatim}
(with \verb+hanauma>+ being the shell's prompt)
This creates the directory \texttt{pgg-1.1} in the current directory.
...
(echo ",bench on"; \
 ...
 for package in pgg-residual pgg ; do \
   echo ",load-package $package";  done ; \
 echo ",open pgg signals"; \
 echo ",open auxiliary pgg-library cogen-specialize pp"; \
 echo ",collect"; \
 echo ",dump pgg.image \"(PGG-1.1 made by $LOGNAME `date`)\"";
 echo ",exit" )  | scheme48 -h 10000000
Welcome to Scheme 48 0.51 (made by pjt on Wed Jun 24 08:54:28 BST 1998).
...
Undefined: (anf-specializer anf-specializer-interface)
> [pp /usr/local/lib/scheme48/big/pp.scm]
[auxiliary auxiliary.scm]
[cogen-gensym cogen-gensym.scm]
[cogen-boxops cogen-boxops.scm]
[cogen-globals cogen-globals.scm]
[cogen-specialize cogen-specialize.scm]
[cogen-library cogen-library.scm]
[shift-reset shift-reset.scm]
[cogen-residual cogen-residual.scm]
[cogen-completers cogen-completer.scm]
[cogen-record cogen-record.scm]
[cogen-memo-standard cogen-memo-standard.scm]
[pgg-library cogen-direct-anf.scm]
[define-data cogen-ctors.scm]
[pgg-residual]
> [cogen-env cogen-env.scm]
[cogen-macro cogen-macro.scm]
[cogen-abssyn cogen-abssyn.scm]
[cogen-scheme cogen-scheme.scm]
[cogen-labset cogen-labset-bylist.scm]
[cogen-bta cogen-effect.scm cogen-eq-flow.scm
Undefined: (parse-type desc-np desc-nc desc-nt desc-type btann)
]
[cogen-typesig cogen-typesig.scm]
[cogen-oca cogen-oca.scm]
[cogen-construct-genext cogen-construct-genext.scm]
[cogen-skeleton cogen-skeleton.scm]
[cogen-terminate cogen-terminate.scm]
[pgg cogen-driver.scm]
> > > Before: 2377611 words free in semispace
After:  3560836 words free in semispace
> Writing pgg.image
> 20.221u 0.572s 0:24.01 86.5%  103+50335k 0+90io 27pf+0w
hanauma>
\end{verbatim}
Do not worry about the \texttt{Undefined:} messages, they result from
mutually recursive modules.

Next time you want to use PGG, type
\begin{verbatim}
hanauma> scheme48 -h 6000000 -i pgg.image
\end{verbatim}
...
bta-solve
bta-solve done
'((define (power x_1 n_1) (_if 0 1 (_op 0 = '0 n_1) (_lift ...
> 
\end{verbatim}
\end{small}
...
(define (power x_1 n_1)
 ...
 (define ($goal x_1 n_1) (power x_1 n_1)))
\end{verbatim}
\end{small}
...
\end{verbatim}
\end{small}
This approach is recommended if the source program does not yet
...
specialize. The \texttt{specialize} function takes as parameters the
binding-time skeleton \texttt{'(\$goal 1 0)} and an
argument template \texttt{'(ignored 0)}. The binding-time skeleton names the
entry point and lists the binding times. The argument template lists \emph{all}
arguments, one for each binding time in the skeleton. The arguments
corresponding to non-zero binding times
must be symbols, because they serve as stubs for variable names. The
arguments corresponding to zero binding times serve as the static inputs.
\begin{small}
\begin{verbatim}
> (specialize $goal '($goal 1 0) '(ignored 0))
'($goal-1 ignored)
> *residual-program*
'((define ($goal-1 ignored-1) 1))
> 
\end{verbatim}
\end{small}
The specializer responds with the call template for the residual
program, \texttt{'(\$goal-1 ignored)}, indicating that \texttt{\$goal-1}
is the entry point of the residual program and that it takes one
parameter. 
The specializer puts the residual program in variable
\indextt{*residual-program*}\texttt{*residual-program*}, for further
...
\texttt{*} is pure and that no code duplication arises from it. Given
an appropriate declaration as in  the file \texttt{"examples/pure-arith.scm"},
\begin{small}
\begin{verbatim}
...
> (load-program genext)
> (specialize $goal '($goal 0 0 1) '(5 () values))
'($goal-1 values)
> (p *residual-program*)
((define ($goal-1 values-1) 5))
32
> (specialize $goal '($goal 0 0 1) '((+ x y) (x y) values))
'($goal-1 values)
> (p *residual-program*)
((define ($goal-1 values-1)
   (let* ((mlet-2 (cdr values-1))
     ...
     (+ mlet-3 mlet-5))))
26
> (specialize $goal '($goal 0 0 1) '((lambda (x) (+ x y)) (y) values))
'($goal-1 values)
> (p *residual-program*)
((define ($goal-1 values-1)
   (define (loop_2-3-2 mlet-3-1)
     (lambda (y_1-2)
       (+ y_1-2 mlet-3-1)))
   (let* ((mlet-2 (cdr values-1))
          (mlet-3 (car values-1)))
...
\texttt{(+ mlet-3 mlet-5)} and \texttt{(+ y\_1-2 mlet-3-1)} is the
corresponding residual code. 

We can get rid of the auxiliary definition of \verb+loop_2-3-2+ by
turning off memoization for the function \texttt{int}. We just change
the first line of its definition to 
...
\end{verbatim}
\end{small}
After constructing a new generating extension, we obtain a
satisfactory residual program.
\begin{small}
\begin{verbatim}
(define ($goal-1 values-1)
  (let* ((mlet-2 (cdr values-1))
 ...
 (define (main d_2)
   (let ((cycle_1
           (_ctor_memo 0 (0 0)
             my-cons
             1
             (_make-cell_memo 0 3 0 (_ctor_memo 0 () my-nil)))))
     (_cell-set!_memo 0
       (_s_t_memo 0 my-cdr cycle_1)
       cycle_1)
     (zip d_2 cycle_1)))
 (define (zip d_1 s_1)
   (multi-memo 1 'zip-4 zip-4 '(1 0) (list d_1 s_1)))
 (define (zip-4 d_1 s_1)
   (_if 1 1
        (_op 1 null? d_1)
        (_lift 0 1 '())
        (_op 1
             ...
             (_op 1
                  cons
                  (_op 1 car d_1)
                  (_lift 0 1 (_s_t_memo 0 my-car s_1)))
             (zip (_op 1 cdr d_1)
                  (_s_t_memo 0 cell-ref (_s_t_memo 0 my-cdr s_1))))))
 (define ($goal d_2)
   (begin
     (_op 0 _define_data '(my-list (my-nil) (my-cons my-car my-cdr)))
     (main d_2))))
>
\end{verbatim}
\end{small}
...
Newly accessible in user: (define-data)
> (load-program genext)
> (specialize $goal '($goal 1) '(x))
'($goal-1 x)
> (p *residual-program*)
((define ($goal-1 x-1)
   (define (zip-4-2 x-1-1)
     (let ((mlet-2 (null? x-1-1)))
       (if mlet-2
         '()
         (let* ((mlet-5 (car x-1-1))
                (mlet-4 (cons mlet-5 1))
                (mlet-6 (cdr x-1-1))
                (mlet-7 (zip-4-2 mlet-6)))
           (cons mlet-4 mlet-7)))))
   (zip-4-2 x-1)))
\end{verbatim}
\end{small}
The cyclic structure vanishes on specialization. The construction of
the pair \texttt{(x . 1)} is implemented by \texttt{(cons mlet-5 1)}.
...
>                              '(dotprod 0 1 2 3)))
> (load-program genext)
> (specialize $goal '($goal 0 1 2 3) '(2 v1 v2 v3))
'(multi-memo 2 '$goal-1 $goal-1 '(0 1 2) (list v1 v2 v3))
\end{verbatim}
  \end{small}
...
\begin{verbatim}
> (p *residual-program*)
((define ($goal-1 v1-3 v2-2 v3-1)
   (let* ((mlet-7 (car v1-3))
          (mlet-8 (_op 1 car v2-2))
          (mlet-6 (_op 1 * (_lift0 1 mlet-7) mlet-8))
          (mlet-9 (_op 2 car v3-1))
          (mlet-5 (_op 2 * (_lift 1 1 mlet-6) mlet-9))
          (mlet-10 (cdr v1-3))
          (mlet-11 (_op 1 cdr v2-2))
          (mlet-12 (_op 2 cdr v3-1))
          (mlet-16 (car mlet-10))
          (mlet-17 (_op 1 car mlet-11))
          (mlet-15 (_op 1 * (_lift0 1 mlet-16) mlet-17))
          (mlet-18 (_op 2 car mlet-12))
          (mlet-14 (_op 2 * (_lift 1 1 mlet-15) mlet-18))
          (mlet-19 (cdr mlet-10))
          (mlet-20 (_op 1 cdr mlet-11))
          (mlet-21 (_op 2 cdr mlet-12))
          (mlet-13 (_op 2 + mlet-14 (_lift0 2 '0))))
     (_op 2 + mlet-5 mlet-13))))
\end{verbatim}
  \end{small}
  ...
  us the name \texttt{\$goal-1} of the entry point.
  \begin{small}
\begin{verbatim}
> (load-program *residual-program*)
> (specialize $goal-1 '($goal-1 0 1 2) '((111 222) v2 v3))
'(multi-memo 1 '$goal-1-1 $goal-1-1 '(0 1) (list v2 v3))
> (load-program *residual-program*)
> (specialize $goal-1-1 '($goal-1-1 0 1) '((333 444) v3))
'($goal-1-1-1 v3)
> (p *residual-program*)
((define ($goal-1-1-1 v3-1)
   (let* ((mlet-2 (car v3-1))
          (mlet-3 (* 36963 mlet-2))
          (mlet-4 (cdr v3-1))
          (mlet-5 (car mlet-4))
...
> (load-program genext)
> (specialize $goal '($goal 0 1) (list  '(cst 555) 'YYY))
> (specialize $goal '($goal 0 1) (list  '(var 555) 'YYY))
> (specialize $goal '($goal 0 1) (list  '(bin (var 1) (var 1)) 'YYY))
> (specialize $goal '($goal 0 1) (list  '(bin (var 1) (bin (cst 4711) (var 1))) 'YYY))
\end{verbatim}
  \end{small}
\end{itemize}
...
\subsection{Specialization with respect to modular data}
\label{sec:modular-data}

It is possible to regard a list of static data as a temporal stream. This
means that only one element of the list is available for every single run of
a specialization. The most striking application for this feature is
the compilation of modular programs by specializing an interpreter.

An an example, we consider the compilation of a simple register machine
language. Here is an example program.
\begin{verbatim}
...
((finis))
\end{verbatim}
The \texttt{main} function of the interpreter accepts four parameters. The
stream of modules, the entry label, the number of registers, and the
initial contents of the registers. All inputs but the last one are assumed to
be statically known. The idea is that the modules (the elements of the stream)
are supplied as they become available. Specialization proceeds until the next
module is required.
\begin{verbatim}
> (define genext (cogen-driver '("examples/modint.scm") '(main 0 0 0 1)))
bta-run
bta-solve
bta-solve done
> ,open pgg-residual
...
> (specialize-$goal "mods" 'add 2)
'(main-1 x4)
\end{verbatim}
Specialization stops right before the first module. So far, it generated code
for transferring the input list into the registers:
\begin{verbatim}
> (p *residual-program*)
((define (main-1 x-1)
   (let* ((mlet-2 (car x-1))
          ...
          (mlet-4 (car mlet-3))
          (mlet-5 (cdr mlet-3)))
     (jump-2 mlet-2 mlet-4))))
\end{verbatim}
The call to \texttt{jump2} calls code that will be generated as soon as the
next module becomes available. This fact is signalled to the system via the
\texttt{continue} function.\indextt{continue}
\begin{verbatim}
> (continue "mods" (list module2))
\end{verbatim}
...
\item $[\tau_1,\dots,\tau_n]\to\tau_0$, for every expression that
...
\begin{verbatim}
D ::= (define-primitive O T [dynamic|error|opaque|pure|apply])
\end{verbatim}
...
property. The type \texttt{T} is currently ignored. The properties
\texttt{dynamic}, \texttt{error}, \texttt{opaque} are synonyms. Each
of them forces the binding time of \texttt{O} to be dynamic. 
...
arguments. The residual program is left in the variable
\texttt{*residual-program*}\dindextt{*residual-program*}. It is a list
...
(continue Name Arg)
\end{verbatim}
The parameter \texttt{Name} identifies the value that the specialization waits
for. It must be equal to the symbolic value used as a placeholder for the
stream value. The example in Section~\ref{sec:modular-data} use the string
\texttt{"mods"} for this purpose.

The parameter \texttt{Arg} is the next value in the stream. The representation
of this value and the indication of the end of the stream is up to the
programmer. The example in Section~\ref{sec:modular-data} use the empty list to 
indicate end-of-stream and a one-element list for the next item in the stream.
...
\item \verb+(set-lambda-is-toplevel! v)+ Default: \texttt{\#f}.
  ...
  Generate a toplevel function for each memoized lambda abstraction if set.
\end{itemize}
