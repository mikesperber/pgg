...
(define anf-next-lambda!
  (lambda (anf)
    (let ((x *anf-pp-count*))
      ...
      anf)))
(define anf-next-vector!
  (lambda (anf)
    (let ((x *anf-pp-count*))
      (set! *anf-pp-count* (+ 1 x))
      (anf-vector->nr! anf x)
      (set! *anf-vectors* (cons anf *anf-vectors*))
      anf)))
(define anf-next-ref!
  (lambda (anf)
    (let ((x *anf-pp-count*))
...
(define anf-next-ctor!
  (lambda (anf)
    (let ((x *anf-pp-count*))
      (set! *anf-pp-count* (+ 1 x))
      (anf-ctor->nr! anf x)
      (set! *anf-ctors* (cons anf *anf-ctors*))
      anf)))
(define anf-next-app!
  (lambda (anf)
    (let ((x *anf-app-count*))
...
(define (anf-extract-maps!)
  ...
  (anf-update-map! *anf-pp-map* anf-vector->nr *anf-vectors*)
  (anf-update-map! *anf-pp-map* anf-ctor->nr *anf-ctors*)
...
;;;			     (loop (annFetchCondElse e) v c)))))
	...
	(anf-convert-e* args v
			(lambda (conv-args)
			  (case (annFetchOpName e)
			    ((make-vector)
			     (anf-next-vector!
			      (make-anf-vector #t conv-args)))
			    ((vector)
			     (anf-next-vector!
			      (make-anf-vector #f conv-args)))
			    ((vector-ref)
			     (make-anf-vector-ref (car conv-args)
						  (cadr conv-args)))
			    ((vector-set!)
			     (make-anf-vector-set (car conv-args)
						  (cadr conv-args)
						  (caddr conv-args)))
			    (else
	...
	(anf-next-let!
     ...
     ((annIsCellEq? e)
      (anf-convert-e* (annFetchCellEqArgs e) v
		      (lambda (conv-rands)
			(make-anf-op 'cell-eq conv-rands))
		      c))
     ((annIsEval? e)
