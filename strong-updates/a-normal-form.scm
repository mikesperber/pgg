...
(define-record anf-ctor (name actuals)	;values -> comp
  nr)
(define-record anf-sel (name field actual)) ;value -> comp
(define-record anf-test (name actual))	;value -> comp

(define-record anf-vector (kind actuals) ;bool value* -> comp
  nr)					;kind=#t: make-vector, kind=#f: vector
(define-record anf-vector-ref (vec index)) ;value value -> comp
(define-record anf-vector-set (vec index actual)) ;value value value -> comp
...
(define (anf-freevars vars anf)
  (let loop ((anf anf))
	...
	(if (labset-elem? var vars)
     ...
     ((anf-vector? anf)
      (labset-union* (map loop (anf-vector->actuals anf))))
     ((anf-vector-ref? anf)
      (labset-union (loop (anf-vector-ref->vec anf))
		    (loop (anf-vector-ref->index anf))))
     ((anf-vector-set? anf)
      (labset-union (loop (anf-vector-set->vec anf))
		    (labset-union
		    (loop (anf-vector-set->index anf))
		    (loop (anf-vector-set->actual anf)))))
     ((anf-ref? anf)
  ...
  (let loop ((anf anf))
    (cond
     ...
     ((anf-test? anf)
      ...
      (loop (anf-test->actual anf)))
     ((anf-vector? anf)
      (if (anf-vector->kind anf)
	  (display "make-vector[")
	  (display "vector["))
      (display (anf-vector->nr anf))
      (display "]")
      (for-each (lambda (actual)
		  (display " ") (loop actual))
		(anf-vector->actuals anf)))
     ((anf-vector-ref? anf)
      (display "vector-ref ")
      (loop (anf-vector-ref->vec anf))
      (display " ")
      (loop (anf-vector-ref->index anf)))
     ((anf-vector-set? anf)
      (display "vector-set ")
      (loop (anf-vector-set->vec anf))
      (display " ")
      (loop (anf-vector-set->index anf))
      (display " ")
      (loop (anf-vector-set->actual anf)))
