...
(define (annMakeOpaqueOp op args)
  (vector 'OP #f #f 0 op args #f #f #f))
(define (annMakeFullOp op opaque property type args)
  (vector 'OP #f #f 0 op args opaque property type))
(define (annMakeOp1 opaque property type)
  (lambda (op args) (annMakeFullOp op opaque property type args)))
(define (annIsOp? e)
...
(define (annMakeCtor1 desc)
  (lambda (ctor args)
    (let loop ((args args) (vars '()))
      (if (null? args)
	  (annMakeCtor ctor desc (map annMakeVar (reverse vars)))
