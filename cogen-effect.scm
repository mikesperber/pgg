	...
	(let ((evar  (make-effect '() empty-labset)))
	  ...
	  (cond
	   ...
	   ((annIsRef? e)
	    ...
	    (for-each (lambda (var) (effect-add-neighbor! var evar))
	   ...
	   ((annIsAssign? e)
	    ...
	    (let* ((ref (annFetchAssignRef e))
		   ...
		   (effects (append (loop ref) (loop arg))))
	      (display 'assign effects) (newline)
	      (for-each (lambda (var) (effect-add-neighbor! var evar)) effects)
...
(define (allocate-effect-variables-obsolete-e e d*)
  (let loop ((e e))
    ;; (display (vector-ref e 1)) (newline)
    (let ((evar  (make-effect '() empty-labset)))
      (annExprSetEffect! e evar)
